...
const getLatestDateandTimestamp = (
...
const filtterInsertValues = (
  values: (string | number)[][],
  lastDate: string,
  lastTimestamp: string
): (string | number)[][] => {
  const tmp = [...values];
  const convertdataAndTimestamp: string[][] = tmp.map((item) =>
    item.map((i) => String(i))
  );
  const dataAndTimestamp = convertdataAndTimestamp.map((item) => [
    item[1],
    item[2],
  ]);
  const lastItemIndex = findIndexDateAndTimestamp(
    dataAndTimestamp,
    lastDate,
    lastTimestamp
  );
...
function findIndexDateAndTimestamp(
  array: string[][],
  lastDate: string,
  lastTimestamp: string
): number {
...
const insertValuesToSheet = async (
  ...
  const insertValues = filtterInsertValues(
    scoreMetrix,
    lastDate,
    lastTimestamp
  );
  insertValues.length === 0
    ? undefined
    : await sheet.addRows(addID(insertValues, rows));
};
...
export { createSheetsInstance, insertValuesToSheet };
